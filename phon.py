...
for p in cypher:
  if p in consonants.keys():
    ...
    V.add(p)
    rules+='v'
with open(sys.argv[1]+'.tex','w') as temp: temp.write('\\footline={\\tenrm\it '+time.strftime('%d.%m.%y')+'\hfill '+time.strftime('%H.%M.%S')+'\hfill\\folio}\chardef\match=\pdfcolorstackinit page direct{0 g} \\font\ipa=tipa17 \pdfpagewidth 5.5in \pdfpageheight 8.5in \hsize 4.5in \\vsize 7in \hoffset -0.5in \\voffset -0.375in\n')
matching=[]
_m=0
print(str(set(''.join(cypher).split('.'))))
while _m<len(''.join(cypher).split('.')):
  phonemes='\\null\\vfill\n'
  for ln in range(len(verse)):
    ...
    for lt in range(len(verse[ln])):
      ...
      sb=''.join(syll)
      if sb in [vr for vr in ''.join(cypher).split('.')]:
        phonemes+='\pdfcolorstack\match push{1 0 0 rg}'
        if len([x for x in matching if x==sb])<len([y for y in ''.join(cypher).split('.') if y==sb]): matching.append(sb)
      for phon in syll:
      ...
      if sb in [vr for vr in ''.join(cypher).split('.')]: phonemes+='\pdfcolorstack\match pop{}'
      if lt<len(verse[ln])-1: phonemes+='\ipa\char"2E'
      elif ln<len(verse)-1: phonemes+='}\\bigskip\n'

  if len(matching)>_m:
    with open(sys.argv[1]+'.tex','a') as temp: temp.write(phonemes+'}\\vfill\eject\n')
    _m=len(matching)

with open(sys.argv[1]+'.tex','a') as temp: temp.write('\\bye')
  #time.strftime("%d%m%y%H%M%S_")
  #for u,h in accents.items(): temp.write('\def\\'+u+'#1{{\\acc\\accent"'+h+' #1}}'+'\n')
  #\\font\\acc=tipa10
os.system('pdftex '+sys.argv[1]+'.tex') # time.strftime("%d%m%y%H%M%S_")