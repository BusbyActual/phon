# the rhyming syllable (and all matching syllables) are matching even when only part of the generated syllable exists in the source phrase. For example, "phan" would be a match for "pha" in the source.
#
#!/usr/bin/env python
...
morphing={'l','r','m','n'} # N can only occur at end of syllable. morphing cannot occur as first of two C's unless the combination occurs in the source
glides={'h','j','w'}
class page:
  def __init__(self):
  ...
  def compose(self):
      ...
      for lt in range(len(self.verse[ln])):
        ...
        while len(syll)<len(rule):
          if rule[len(syll)]=='c':
            ...
            else: con=random.choice([ph for ph in self.C if ph not in syll])
            pr=con
          else:
    ...
    if len([s for s in syllables if s in self.sylbs])>0:
      print('page '+str(self.f))
      self.f+=1
      with open('log','a') as log: log.write(' '.join([s for s in syllables if s in self.sylbs])+'\n')
      return phonemes+'}\n\\vfill\eject\n'
    else: return ''
pg=page()
def main():
  with open('log','w') as log: log.write('matching syllables:\n')
